...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/registry.py --seats-dir ${CLAUDE_PLUGIN_ROOT}/seats [--domain <tag>]
```
...
- If the exit code is 2 (no seats directory): tell the user to run `/chiron:hire` or install a pack.
